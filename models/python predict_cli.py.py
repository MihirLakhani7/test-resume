...
# Example: Feature Importance (if your model is a tree with .feature_importances_)
# If your model is a tuple-based tree, you may need to compute importances manually or skip this.

# Example: Distribution of predictions (simulate with random/sample data)
# Replace this with your real data if available
sample_predictions = np.random.choice([0, 1], size=100, p=[0.7, 0.3])
plt.figure(figsize=(5,3))
plt.title("Cardio Risk Prediction Distribution")
plt.bar(['Low Risk', 'At Risk'], [np.sum(sample_predictions==0), np.sum(sample_predictions==1)], color=['#4caf50', '#e53935'])
plt.ylabel("Count")
plt.tight_layout()
plt.savefig("models/cardio_pred_distribution.png")
plt.close()

# Example: Age distribution (simulate)
...
plt.ylabel("Count")
plt.tight_layout()
plt.savefig("models/cardio_age_distribution.png")
plt.close()

# Example: BMI distribution (simulate)
...
plt.ylabel("Count")
plt.tight_layout()
plt.savefig("models/cardio_bmi_distribution.png")
plt.close()